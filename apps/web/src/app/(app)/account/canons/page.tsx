...
function CanonPill({ label, present }: { label: string; present: boolean }) {
  ...
  return (
    <span
      className={`rounded border px-1.5 py-0.5 text-[10px] uppercase tracking-wide ${cls}`}
    >
      {label}
    </span>
  );
    ...
    <div className="px-8 py-8">
      <header className="mb-6 max-w-3xl">
        ...
        <p className="mt-1 text-sm text-zinc-400">
          Cross-project conventions, guidelines and architecture grouped by domain.
          A consultant working on more than one practice area (e.g. Salesforce and
          digital narratives) keeps each body of canon completely separate. Each
          project belongs to one domain and inherits its canon as the default;
          project-level canon overrides where set.
        </p>
      </header>
      ...
      <section className="mb-8 max-w-3xl">
        ...
        {domains.length === 0 ? (
          <div className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-6 text-sm text-zinc-500">
            You haven't created any canon domains yet. Create one below — start with
            something like <code className="font-mono text-zinc-300">salesforce</code>{" "}
            or <code className="font-mono text-zinc-300">digital-narratives</code>.
          </div>
        ) : (
      ...
      <section className="max-w-3xl">
        ...
        <p className="mb-3 text-xs text-zinc-500">
          A domain groups projects that share cross-project canon. After creating it
          you can edit conventions, guidelines and architecture, and assign projects to
          it from each project's editor.
        </p>
